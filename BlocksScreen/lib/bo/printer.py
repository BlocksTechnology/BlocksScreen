...
class Printer(QObject):
    ...
    def object_list_received(self, object_list: list):
        ...
        self.toolhead_number_signal.emit(self.extruder_number)
    ...
    def report_received(self, report: list) -> None:
        ...
        _logger.debug(
            f"Object report received for {_objects_updated_names} objects, going to callbacks"
        )
    ...
    ####################*# Callback Related #*####################
    # TODO : Names for the objects must be passed, ex:. "extruder", "extruder1", this is actually not hardcoded
    # TODO: Extruder name, right now is different, but i want to use the same method for all extruders.

    #####*## Callbacks for the objects ##*#####
    @pyqtSlot(list, name="gcode_response_report_received")
    ...
    def gcode_macro_object_updated(self, values: dict, gcode_macro_name: str) -> None:
        ...
        # TODO: Handle received information from macros here.
    ...
    def unload_filament_object(
        self, values: dict, name: str
    ) -> None: ...  # TODO Add unload filament object verification
    def load_filament_object(
        self, valus: dict, name: str
