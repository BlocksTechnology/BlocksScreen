...
# The repo this updater runs from, and the state entry recording its HEAD as of
# the daemon's last start. Component names must start with an alphanumeric, so
# the key cannot collide with a component; the entry is a dict like every other
# one because the self-heal reconcile drops non-dict entries.
_UPDATER_REPO = Path(__file__).resolve().parent.parent
_UPDATER_STATE_KEY = "_updater"
...
class UpdateService:
    ...
    async def pending_provision(self) -> list[ComponentConfig]:
        ...
        async with self._state_lock:
            ...
            entry = _ensure_comp(state, _UPDATER_STATE_KEY)
    ...
    async def _provision_component(self, component: ComponentConfig) -> bool:
        ...
        try:
            ...
            if component.service:
                ...
                unit_pending = not svc_ok and await service_unit_missing(
                    component.service
                )
                if unit_pending:
            ...
            if component.service and not unit_pending:
