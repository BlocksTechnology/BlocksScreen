...
class UpdateService:
    ...
    async def _safe_revert(self, component: ComponentConfig, prev_hash: str) -> bool:
        """Reset one repo, containing a raise so it cannot skip the rest of the recovery."""
        try:
    ...
    async def _safe_restart(self, service: str, health_url: str | None = None) -> bool:
        """Restart one service, containing a raise so it cannot skip the rest of the recovery."""
        try:
            return await self._restart_one(service, health_url)
        except Exception:  # noqa: BLE001
            self._log.error("%s: restart raised", service, exc_info=True)
            return False
    ...
    async def _abort_batch(
        ...
        for c in touched:
            if not await self._safe_revert(c, prev[c.name]):
        ...
        for service in restarted:
            if not await self._safe_restart(service):
                restart_ok = False
    ...
    async def _run_apt_update(self, component: ComponentConfig) -> bool:
        ...
        async with self._apt_lock:
            return await self._run_apt_update_locked(component, _APT_STATUS_CACHE)
