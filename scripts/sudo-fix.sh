...
SUDOERS_FILE='/etc/sudoers.d'
SUDOERS_FILE='021-sudo-for-blocksscreen'
NEW_GROUP='blocksscreensudo'
...
Cmnd_Alias SYSTEMCTL_BS_RESTART = /bin/systemctl restart BlocksScreen 


%GROUPNAME  ALL=(ALL) NOPASSWD: IWCONFIG, IWLIST, SYSTEMCTL_BS_RESTART
#EOF
...
function check_update_sudoers_file(){
    ...
    then 
        ...
        if [ -e "$SUDOERS_DIR/$SUDOERS_FILE" ]; 
            then 
                ...
                report_status "\e[1;32mFile Not found:\e[0m ${SUDOERS_FILE}"
                create_sudoers_file
                install_sudoers_file
        fi 
        ...
        if [ $(groups | grep -c "$NEW_GROUP") -eq 1 ]; 
            then 
                ...
                report_status "Adding User ${USER} to Groups $NEW_GROUP...."
                add_user_to_group
                adduser_hint
        fi 
fi 
