...
# TODO: Change from print to loggers
...
class SdbusNetworkManager:
    ...
    def toggle_wifi(self, toggle: bool):
        ...
        if not isinstance(toggle, bool):
            raise ValueError("Correct type should be a boolean")
    ...
    def get_current_ip_addr(self) -> typing.List[str] | None:
        ...
        if _device_ip4_conf_path == "/":
            ...
            print("NetworkManager reports no ip config for the interface")
            # TODO: Logging
            return None
                ...
                known_networks.append(
                    {
                        ...
                        "SECURITY_TYPE": conn["802-11-wireless-security"]["key_mgmt"][1].decode(),
                        "CONNECTION_PATH": connection,
                    }
                )
    ...
    def add_wifi_network(self, ssid: str, psk: str) -> typing.Dict:
        ...
        if ssid in _available_networks.keys():
            ...
            if "security" in _wanted_network.keys():
                ...
                return {"status": "error", "msg": "No security type for network, stopping"}
            if _security_types[0] is None:
                ...
                if (
                    ...
                    return {"status": "error","msg": "Security type P2P_TKIP OR BRADCAST_TKIP not supported"}
                    ...
                    # * OWE TM
                    # raise NotImplementedError
                    return {"status": "error","msg": "Security type  AUTH_OWE_TM not supported"}
                    ...
                    # * EAP SUITE B
                    # raise NotImplementedError
                    return {"status": "error","msg": "Security type EAP_SUITE_B not supported"}
                ...
                try:
                    ...
                    return {"status": "exception","msg": f"Exception occurred could not connect to network: {e} "}
                except (
                    NmConnectionPropertyNotFoundError
                    or NmConnectionInvalidPropertyError
                ) as e:
                    return {"status": "exception","msg": f"Error configuring properties for the network, internal error  : {e}"}
                except Exception as e:                    
                    return {"status": "exception","msg": f"Could not connect to network: {e}"}
    ...
    def delete_network(self, ssid: str) -> typing.Dict | str:
        ...
        try:
            ...
            raise Exception(f"Unknown Error detected exception: {e}")
            return {"status": "error"}
    ...
    def create_hotspot(
        ...
        try:
            ...
            print(f"Error occured while adding a hotspot connection: {e.args}")
