...
class TestProvisionMissingComponent:
    ...
    async def test_provision_with_uninstalled_unit_defers_setup(
        ...
        with (
            ...
            patch("updater.service.restart_service", return_value=(False, "no unit")),
            patch("updater.service.wait_for_service_active") as mock_wait,
            patch("updater.service.enable_service") as mock_enable,
            patch("updater.service.shutil.rmtree") as mock_rmtree,
        ...
        assert ok is True
        mock_wait.assert_not_called()
        mock_enable.assert_not_called()  # install-updater enables it
...
class TestPendingProvision:
    ...
    async def test_new_updater_head_returns_missing_opted_in(self, tmp_path):
        ...
        assert svc._read_state()["_updater"] == {"head": "newhead"}

    @pytest.mark.asyncio
    async def test_same_head_is_plain_reboot(self, tmp_path):
        svc = self._svc(tmp_path)
        svc._write_state({"_updater": {"head": "samehead"}})
    ...
    @pytest.mark.asyncio
    async def test_head_entry_survives_self_heal_reconcile(self, tmp_path):
        # _reconcile_self_heal_state drops non-dict entries; the HEAD record must
        # survive it, or every boot would look like a fresh self-update.
        svc = self._svc(tmp_path)
        with patch("updater.service.git_get_hash", return_value="newhead"):
            assert await svc.pending_provision()
    ...
    def test_head_entry_does_not_break_nrestarts_priming(self, tmp_path):
        svc = self._svc(tmp_path)
        svc._write_state({"_updater": {"head": "abc"}})
        svc._prime_nrestarts_sample_ring()  # must not raise on the entry
