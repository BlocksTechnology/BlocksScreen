...
class Bucket:
    def __init__(self, config):
        ...
        self.bucket_position = config.getfloatlist("bucket_position_xy", None, count=2)
    ...
    def move_to_bucket(self, split: typing.Optional["bool"] = False):
        ...
        if self.toolhead is None:
            return
        try:
            # * Check if the bucket is outside the printer boundary
            if self.custom_bed_bound_object is not None:
                _conf_bound = self.custom_bed_bound_object.check_boundary_limits(
                    position=(self.bucket_position[0], self.bucket_position[1])
                )
                if (
                    not _conf_bound["x"] or not _conf_bound["y"]
                ) and self.custom_bed_bound_object.get_status()["status"] == "custom":
            ...
            if (
                self.custom_bed_bound_object is not None
                and self.custom_bed_bound_object.get_status()["status"] == "default"
            ):
                self.custom_bed_bound_object.set_custom_boundary()
        except Exception as e:
            # self.gcode.respond_error(
            #     "Exception encountered while trying to move to bucket position"
            # )
            logging.info("Exception occurred when moving toolhead to bucket position.")
